...
interface UseProblemActionsProps {
    ...
    setProblemData: React.Dispatch<React.SetStateAction<any>>;
    setUserProblemData: React.Dispatch<React.SetStateAction<any>>;
}
            ...
            try {
                await runTransaction(firestore, async (transaction) => {
                    ...
                    if (type === 'star') {
                        ...
                        const newStarred = action === 'add'
                            ? [...starredProblems, problemId]
                            : starredProblems.filter((id: string) => id !== problemId);

                        transaction.update(userRef, { starredProblems: newStarred });
                        setUserProblemData((prev: { liked: boolean; starred: boolean }) => ({
                            ...prev,
                            starred: action === 'add'
                        }));
                        return;
                    ...
                    if (problemDoc.exists()) {
                        ...
                        const newLikes = action === 'add' ? currentLikes + 1 : currentLikes - 1;
                        const newLiked = action === 'add'
                            ? [...likedProblems, problemId]
                            : likedProblems.filter((id: string) => id !== problemId);

                        transaction.update(userRef, { likedProblems: newLiked });
